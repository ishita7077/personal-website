...
export async function uploadNoteImage(
  ...
  try {
    ...
    if (!supabase) {
      logger.warn("image-upload", "No Supabase; upload skipped");
    ...
    if (error) {
      logger.error("image-upload", "Supabase storage upload failed", {
        fileName,
        noteId,
        error,
      });
    ...
    logger.error("image-upload", "Unexpected error during upload", { error });
    return {
      success: false,
