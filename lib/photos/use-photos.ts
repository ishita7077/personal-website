...
export function usePhotos(): UsePhotosResult {
  ...
  const fetchPhotos = useCallback(async () => {
    try {
      ...
      if (!supabase) {
        logger.warn("use-photos", "No Supabase; skipping photo fetch");
        setLoading(false);
        return;
      }
      ...
      if (fetchError) {
        logger.error("use-photos/fetchPhotos", "select_photos failed", fetchError);
      ...
      setError(message);
      logger.error("use-photos/fetchPhotos", "Error fetching photos", { error: err });
    } finally {
      setLoading(false);
    }
  }, []);
