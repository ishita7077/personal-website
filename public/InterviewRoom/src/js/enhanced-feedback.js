...
(function (global) {
  ...
  const API_PATH = '/api/interview-room/enhanced-feedback';
  ...
  function requestEnhancedAnswerFeedback(index, callback) {
    ...
    function doRequest() {
      ...
      const expectedAnswerMap = buildExpectedAnswerMap(haas);
    ...
    for (let i = 0; i < (questions.length || 0); i++) {
      ...
      if (q && typeof t === 'string') {
        ...
        const haas = (typeof IR.getHaasRecommendation === 'function' && q.text)
          ? IR.getHaasRecommendation(q.text)
          : null;
        const expectedAnswerMap = haas ? buildExpectedAnswerMap(haas) : undefined;
        answers.push({
