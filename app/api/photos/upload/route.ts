...
function getServiceClient() {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    logger.error("api/photos/upload", "Missing SUPABASE_URL or SERVICE_ROLE_KEY");
    return null;
  }
...
async function categorizeImage(base64Image: string): Promise<string[]> {
  if (!process.env.OPENAI_API_KEY) {
    logger.warn("api/photos/upload/categorizeImage", "OPENAI_API_KEY not set, skipping categorization");
    return [];
  }
...
Do not include any other text, just the category name.`,
    ...
    logger.error("api/photos/upload/categorizeImage", "Error categorizing image", { error });
    return [];
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      logger.error("api/photos/upload", "Supabase not configured (missing env)");
      return NextResponse.json(
        { error: "Photos upload not configured" },
        { status: 503 }
      );
    }
    ...
    if (uploadError) {
      logger.error("api/photos/upload", "Storage upload failed", uploadError);
      return NextResponse.json(
    ...
    if (insertError) {
      logger.error("api/photos/upload", "Database insert_photo failed", insertError);
    ...
    logger.error("api/photos/upload", "Unexpected error", { error });
