...
// (Removed original SF restaurant/date-night list; calendar now uses generic dinners)
...
// All patterns end by 17:30 to allow 30-min buffer before evening plans/dinner
...
function generateSampleEventsForDay(day: Date): CalendarEvent[] {
  ...
  if (isWeekday) {
    ...
    if (isSaturday) {
      // Generic Saturday evening block (no specific restaurant / date night)
      events.push({
        id: `sample-saturday-evening-${dateStr}`,
        title: "evening",
        startDate: dateStr,
        endDate: dateStr,
        startTime: "18:30",
